...
public class CacheUtilDB {
						...
						try {
							return (List<BackofficeDettaglioStreamDatasetResponse>)
									AdminDBClientDelegate.getInstance().getDatasetService().selectDatasetByOrganizationCode(
											key.getOrganizationCode(), true).getObject();
						} 
						catch (NotFoundException e) {
							return null;
						}
	...
	// ######################################################################################################
		private static Cache<KeyCacheDB, List<BackofficeDettaglioStreamDatasetResponse>> listaStreamDatasetByOrganizationCodeIconVisibilityCache;
		static {
			listaStreamDatasetByOrganizationCodeIconVisibilityCache = new Cache2kBuilder<KeyCacheDB, List<BackofficeDettaglioStreamDatasetResponse>>(){}
					.resilienceDuration(RESILIENCE_DURATION, RESILIENCE_TIME_UNIT)
					.expireAfterWrite(EXPIRE_AFTER_WRITE_DURATION, EXPIRE_TIME_UNIT)
					.refreshAhead(REFRESH_AHEAD)
					.retryInterval(RETRY_INTERVAL_DURATION, TimeUnit.SECONDS)
					...
					.loader(new CacheLoader<KeyCacheDB, List<BackofficeDettaglioStreamDatasetResponse>>() {
						...
						public List<BackofficeDettaglioStreamDatasetResponse> load(KeyCacheDB key) throws Exception {
							...
							try {
								return (List<BackofficeDettaglioStreamDatasetResponse>)
										AdminDBClientDelegate.getInstance().getDatasetService().selectDatasetByOrganizationCode(
												key.getOrganizationCode(), key.getIconRequested()).getObject();
							} 
							catch (NotFoundException e) {
								return null;
							}
		...
		public static List<BackofficeDettaglioStreamDatasetResponse> getListStreamDatasetIconVisibility(KeyCacheDB keyCache) throws Exception {
			return listaStreamDatasetByOrganizationCodeIconVisibilityCache.get(keyCache);	
		}

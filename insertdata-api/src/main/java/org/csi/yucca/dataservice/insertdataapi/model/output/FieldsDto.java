...
//            "(\\d{2}):(\\d{2})\\.(\\d{3})?)((\\+|-)(\\d{2}):?(\\d{2}))?");	
	...
	public FieldsDto (String fieldName,String fieldType,long datasetId,long datasetVersion, boolean required) throws Exception{
		this.setFieldName(fieldName);
		this.setFieldType(fieldType);
	...
	{
		return validateValue(valueToCheck, !isRequired());
	}

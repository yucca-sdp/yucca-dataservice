...
public class Krb5HttpClientConfigurer extends HttpClientConfigurer {
	...
	private static final Logger logger = LoggerFactory.getLogger("it.csi.smartdata.dataapi.solr.Krb5HttpClientConfigurer");
	...
	public void configure(DefaultHttpClient httpClient, SolrParams config) {
		...
		if (true || System.getProperty("java.security.auth.login.config") != null) {
			...
			if (configValue != null) {
				logger.debug("Setting up SPNego auth with config: "
						+ configValue);
				String useSubjectCredsProp = "javax.security.auth.useSubjectCredsOnly";
				String useSubjectCredsVal = System
						.getProperty("javax.security.auth.useSubjectCredsOnly");
				logger.debug("useSubjectCredsVal: "
						+ useSubjectCredsVal);
	...
	private static class SolrJaasConfiguration extends Configuration {
		...
		public SolrJaasConfiguration() {
			try {
				Krb5HttpClientConfigurer.logger.debug("Constructor base");
		...
		public SolrJaasConfiguration(String clientName) {
			...
			try {
				Krb5HttpClientConfigurer.logger.debug("Constructor: "+clientName);
				this.baseConfig = Configuration.getConfiguration();
			} catch (SecurityException e) {
				this.baseConfig = null;
			}
		...
		public AppConfigurationEntry[] getAppConfigurationEntry(String appName) {
			...
			Krb5HttpClientConfigurer.logger.debug("InitiateAppName contains?: "
					+ this.initiateAppNames.contains(appName));


			if (true || this.initiateAppNames.contains(appName)) {
				Krb5HttpClientConfigurer.logger.debug("Using AppConfigurationEntry for appName '"
						+ clientAppName + "' instead of: " + appName);
				if (this.baseConfig.getAppConfigurationEntry(clientAppName)!=null &&
						this.baseConfig.getAppConfigurationEntry(clientAppName).length>0)
				{
					for (int i = 0; i < this.baseConfig.getAppConfigurationEntry(clientAppName).length; i++) {
						AppConfigurationEntry appconf = this.baseConfig.getAppConfigurationEntry(clientAppName)[i];
						Krb5HttpClientConfigurer.logger.debug("LoginModuleName"+appconf.getLoginModuleName());
						Krb5HttpClientConfigurer.logger.debug("ControlFlag"+appconf.getControlFlag());
						if (appconf.getOptions()!=null)
						{
							Iterator iter = appconf.getOptions().entrySet().iterator();
							while (iter.hasNext()) {
								Map.Entry opt = (Map.Entry) iter.next();
								Krb5HttpClientConfigurer.logger.debug("Opt:"+opt.getKey()+"->"+opt.getValue());
							}
						}

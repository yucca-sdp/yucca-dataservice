...
public class SDPOdataRuparFilter implements Filter{

	static Logger log = Logger.getLogger(SDPOdataRuparFilter.class.getPackage().getName());

	static Logger logAccounting= Logger.getLogger("accounting");

	@Override
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws ServletException, IOException {
		log.info("[SDPOdataRuparFilter::doFilter] BEGIN");
...
//	    	}
			...
			log.debug("[SDPOdataRuparFilter::doFilter] requestURI="+requestURI);
			...
			log.info("[SDPOdataRuparFilter::doFilter] webFilterPattern="+webFilterPattern);
			log.info("[SDPOdataRuparFilter::doFilter] webServletUrl="+webServletUrl);

			
			webFilterPattern="/SmartDataOdataRuparService.svc/";
					
			if (requestURI.indexOf(webFilterPattern)!=-1) {
				log.info("[SDPOdataRuparFilter::doFilter] FILTERING");
				...
				//if (dopo.length()>0) codiceApi=codiceApi.substring(0, codiceApi.indexOf(dopo)-1);
				//else codiceApi=codiceApi.substring(0, codiceApi.length()-1);
				String newURI=webServletUrl+dopo+"?apiContext=odatarupar&codiceApi="+codiceApi+"&apacheUniqueId="+uniqueid;
				...
				log.info("[SDPOdataRuparFilter::doFilter] codiceApi="+codiceApi);
				log.info("[SDPOdataRuparFilter::doFilter] newURI="+newURI);
			...
			log.error("[SDPOdataRuparFilter::doFilter] " +e);
			...
			log.debug("[SDPOdataRuparFilter::doFilter] END");

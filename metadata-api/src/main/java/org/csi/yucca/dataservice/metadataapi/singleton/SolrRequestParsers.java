...
public class SolrRequestParsers {
	...
	private static final byte[] INPUT_ENCODING_BYTES = INPUT_ENCODING_KEY
	 ...
	 *            as given from URL
	 * @param map
	 *            place all parameters in this map
	 */
	static void parseQueryString(final String queryString,
		...
		if (queryString != null && queryString.length() > 0) {
			try {
				...
				// this input stream emulates to get the raw bytes from the URL
				// as passed to servlet container, it disallows any byte > 127
				// and enforces to %-escape them:
				...
				parseFormDataContent(in, Long.MAX_VALUE,
						StandardCharsets.UTF_8, map, true);
	...
	/**
	 * Given a url-encoded form from POST content (as InputStream), map it into
	 * the given map. The given InputStream should be buffered!
	 * 
	 * @param postContent
	 *            to be parsed
	 * @param charset
	 *            to be used to decode resulting bytes after %-decoding
	 * @param map
	 *            place all parameters in this map
	 */
	@SuppressWarnings({ "fallthrough", "resource" })
	static long parseFormDataContent(final InputStream postContent,
			final long maxLen, Charset charset,
			final Map<String, String[]> map, boolean supportCharsetParam)
			throws IOException {
		CharsetDecoder charsetDecoder = supportCharsetParam ? null
				: getCharsetDecoder(charset);
		final LinkedList<Object> buffer = supportCharsetParam ? new LinkedList<>()
				: null;
		...
		for (;;) {
			...
			case '&': // separator
				if (keyStream.size() > 0) {
					final byte[] keyBytes = keyStream.toByteArray(), valueBytes = valueStream
							.toByteArray();
					if (Arrays.equals(keyBytes, INPUT_ENCODING_BYTES)) {
						...
						if (charsetDecoder != null) {
							throw new SolrException(
									...
									supportCharsetParam ? ("Query string invalid: duplicate '"
						...
						charset = Charset.forName(decodeChars(valueBytes,
								keyPos, getCharsetDecoder(CHARSET_US_ASCII)));
						...
						// we already have a charsetDecoder, so we can directly
						// decode without buffering:
						final String key = decodeChars(keyBytes, keyPos,
								charsetDecoder), value = decodeChars(
								valueBytes, valuePos, charsetDecoder);
		...
		// if we have not seen a charset declaration, decode the buffer now
		// using the default one (UTF-8 or given via Content-Type):
	...
	private static String decodeChars(byte[] bytes, long position,
	...
	private static void decodeBuffer(final LinkedList<Object> input,
			final Map<String, String[]> map, CharsetDecoder charsetDecoder) {
		for (final Iterator<Object> it = input.iterator(); it.hasNext();) {
			...
			MultiMapSolrParams.addParam(
					decodeChars(keyBytes, keyPos.longValue(), charsetDecoder)
							.trim(),
					decodeChars(valueBytes, valuePos.longValue(),
							charsetDecoder), map);
		}
	}
	...
	private static int digit16(int b) {
		...
		throw new SolrException(ErrorCode.BAD_REQUEST,
				"URLDecoder: Invalid digit (" + ((char) b)
						+ ") in escape (%) pattern");
	}
}
